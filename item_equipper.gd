...
func _ready():
	...
	# Create timer for 3-second pickup delay
	pickup_delay_timer = Timer.new()
	pickup_delay_timer.one_shot = true
	pickup_delay_timer.wait_time = 1.0
	...
	animation_complete_timer.wait_time = 1.0  # Adjust based on your animation length
...
func _on_global_var_timer_timeout():
	...
	Global_Variables.equip_item = false

func _on_pickup_delay_timeout():
	# After 3 seconds, set the global variable to actually pick up the item
	Global_Variables.equip_item = true
