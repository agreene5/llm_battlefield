...
const TIMEOUT_DURATION = 30.0  # 1 minute in seconds
...
func find_nearest_health_box():
	...
	var health_boxes = parent.get_tree().get_nodes_in_group("Health_Box")
...
func start(action = ""):
	...
	active = true
	stop_called = false
	timer = 0.0  # Reset timer when starting
...
func stop():
	active = false
	stop_called = true
	timer = 0.0  # Reset timer when stopping
